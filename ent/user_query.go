...
func (_q *UserQuery) QueryBackendUsers() *BackendUserQuery {
...
func (_q *UserQuery) WithBackendUsers(opts ...func(*BackendUserQuery)) *UserQuery {
	query := (&BackendUserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBackendUsers; query != nil {
		if err := _q.loadBackendUsers(ctx, query, nodes,
			func(n *User) { n.Edges.BackendUsers = []*BackendUser{} },
			func(n *User, e *BackendUser) { n.Edges.BackendUsers = append(n.Edges.BackendUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadBackendUsers(ctx context.Context, query *BackendUserQuery, nodes []*User, init func(*User), assign func(*User, *BackendUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.BackendUser(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BackendUsersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_backend_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_backend_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_backend_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
